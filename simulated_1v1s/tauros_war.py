...
def t3_select_move(user, opponent):
    attack = None
    if user.hp <= 180:
        attack = user.get_move(HyperBeam.NAME)
    elif user.status == "BRN":
        attack = user.get_move(Blizzard.NAME)
    elif opponent.hp >= 170:
        attack = user.get_move(BodySlam.NAME)
    else:
        attack = user.get_move(HyperBeam.NAME)
    return attack
...
for _ in range(0, iterations):
    ...
    if len(winner) == 2:
        ...
        t1_wins += 1
    logging.debug("================")
t1_percentage = round(t1_wins / float(iterations) * 100, int(math.log(iterations / 100, 10)))
t2_percentage = round(
    (iterations - t1_wins - draws) / float(iterations) * 100, int(math.log(iterations / 100, 10))
)
draw_percentage = round(draws / float(iterations) * 100, int(math.log(iterations / 100, 10)))
